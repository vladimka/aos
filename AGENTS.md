...
- **Test script**: `python3 /tmp/opencode/guitester.py` — dumps PPM screenshots to `/tmp/aos-G*.ppm`, checks pixel colors. `QEMU` monitor socket at `/tmp/aos-gui.sock`. Serial log at `/tmp/aos-gui.log`.
