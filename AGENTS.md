...
- **vgu_irq chains into `virtio_irq_dispatch`**: virtio-gpu shares IRQ 11 with vblk/vrng/vnet (QEMU assigns one PCI INTx line per bus slot-group). `vgu_init`'s `irq_install_handler(11, vgu_irq)` would otherwise overwrite the shared dispatch that services the other virtio devices, silently killing disk I/O (configtest Boot B hung after GPU init until this was fixed). `vgu_irq` acks its own ISR then calls `virtio_irq_dispatch()` (non-static, `drivers/virtio.c`), which polls every registered device's ISR.
...
- **IRQ**: `vgu_irq` acks the device ISR then calls `virtio_irq_dispatch()` (see WM notes above — vblk/vrng/vnet share IRQ 11). Without the chain, `irq_install_handler(11, vgu_irq)` would overwrite the shared dispatch and kill disk I/O.
