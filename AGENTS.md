...
All commands (`help`, `uptime`, `clear`, `echo`, `tick`, `info`, `reboot`, `panic`, `ls`, `cat`, `rm`, `shutdown`, `format`, `test`, `cp`, `mv`, `mkdir`, `rmdir`, `head`, `wc`, `ps`) are **standalone ELF32 programs** under `programs/`:
...
### USB HID tablet (absolute pointer)

- **Why**: AnyDesk/QEMU GTK relative PS/2 deltas desync from the real pointer, so the guest cursor is at (0,0) when a button is pressed → clicks at origin. Fix: QEMU **USB HID tablet** (`usb-tablet`, absolute X/Y 0..0x7FFF) + guest UHCI/HID driver feeding `mouse_x/mouse_y` directly.
- **Guest driver** (`drivers/uhci.c` `uhci_tablet_poll`): a single interrupt-IN TD polled from the **kernel main `hlt` loop** (no UHCI IRQ — `USBINTR_IOC` never enabled, IRQ 11 stays off). TD `link=0x0001`, token = `uhci_explen(8)|(1<<15)|(devaddr<<8)|USB_PID_IN`, buffer `tablet_buf[8]`, `intr_qh.element=&tablet_td`. On NAK the TD stays ACTIVE (retried each frame); on data ACTIVE is cleared, `ACTLEN = len-1` → report is 6 bytes. After every completion (or when idle) the TD is **re-armed** unconditionally, so reports are never lost by the guest.
- **Report format**: `[0]`=buttons (bit0 L, bit1 R, bit2 M), `[1-2]`=X LE, `[3-4]`=Y LE, `[5]`=wheel signed (+1 = wheel up in QEMU's convention). `tablet_present` is set in `usb_init()` after enumeration; `uhci_tablet_poll` no-ops without it.
- **`mouse_tablet_set()`** (`drivers/mouse.c`): scales to the framebuffer (`mouse_x=(x*(mouse_xmax+1))/32768`, Y likewise, `xmax/ymax` default 1023/767), clamps, stores `mouse_buttons=buttons&0x07`, and inverts the wheel sign (`mouse_wheel += -wheel`, `wheel_acc += ±3`) to the PS/2 convention (+ = down). Flips `tablet_active` on the first report (`USB tablet mouse active.`). **PS/2 stays authoritative until then**: `mouse_process_byte()` returns early while `tablet_active`.
- **QEMU facts**: each QMP `input-send-event` ends with a sync, and a report only carries the position from that same call → a release must re-send the abs X/Y. `{"type":"sync"}` inside the events array is **invalid** (`Parameter 'type' does not accept value 'sync'`). QEMU's `hid_pointer_sync` **compresses** a new event into the previous unconsumed one when `n>0` and buttons are unchanged, and under TCG a slow guest can merge/absorb clicks — tests must confirm each action by its on-screen result and retry (see below).
- **Harness**: `scripts/tablettest.py` (`python3 scripts/tablettest.py`) drives the WM entirely via QMP abs events (QTest gained `qmp_socket=`/`qmp()`); `make run` and `scripts/qemu-debug.sh` add `-device piix3-usb-uhci -device usb-tablet`. Registered in the Makefile `TESTS` list. Retries each click until the menu state matches (power menu open → desktop left-click closes it → right-click opens the context menu) and asserts the context menu's accent frame sits exactly at the clamped click point ((900,500) and (848,508) for a (850,500) click), proving abs coords map 1:1.
...
- **`ps`** (`programs/musl/ps.c`): lists the process table by iterating `/proc` numeric entries and reading the per-pid `cmdline`/`status` files (raw `open`/`read` — musl `fopen`'s FILE layer does not work on these procfs files, use syscalls like `cat`/`bgspawn` do). Columns: `PID PPID STATE ABI CMD`; states render as `ready/run/sleep/wait/zomb/spawn`, ABI as `aos/linux`. Serial-console commands run in-place as pid 0, which procfs deliberately does not list — ps cannot show itself there (GUI-term children get real pids and do show).
- **procfs per-pid files** (`kernel/procfs.c`): each `/proc/<pid>/` dir now exposes `trace` (ino base 0x2000), `cmdline` (0x3000, `<name>[ <args>]`) and `status` (0x4000, `State:`/`PPid:`/`Abi:` lines). The name comes from a new `char name[28]` field in `struct task`, filled in `task_spawn` from the spawn-path basename. Renderers use their own static buffers (the shared `proc_content` buf is only 128 B; cmdline can reach ~283 B). Test: `scripts/pstest.py` (in `TESTS`).
