...
- [x] **P0 — RTC (CMOS) для реального времени.** `drivers/rtc.c`: читать часы/дату из CMOS `0x70/0x71`; будильник. Сейчас `clock` показывает время с момента загрузки, а не стенное. (готово: `rtc.c` читает CMOS c BCD/бинарным разбором и UIP-поллингом; будильник не нужен)
