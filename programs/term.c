...
// The text cursor row (crow) starts with the "aos> " prompt; the command
// line is the part after it. ccol must not move left of the prompt.
...
static void render(void) {
    ...
    utfbuf[pos] = 0;
    render_text(win, (unsigned int)w * 4, 0, 0, utfbuf, COL_FG, COL_BG);
    fill_rect(win, (unsigned int)w * 4, ccol * FONT_W, crow * FONT_H + 14,
              FONT_W, 2, COL_FG);
    struct aos_msg m = {MSG_UPDATE, winid, 0, 0, 0};
    send_msg(get_event_pid(), &m);
}

// Extract the current command line (ASCII only): the text after the prompt.
