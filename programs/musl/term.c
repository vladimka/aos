...
#define SB_LINES 512
static struct tcell hist[SB_LINES][TW];
static unsigned int htotal;
static int voff;
...
static void sb_clamp(void) {
    int avail = (htotal < SB_LINES) ? (int)htotal : SB_LINES;
    if (voff > avail) voff = avail;
    if (voff < 0) voff = 0;
}

static const struct tcell *view_row(int r) {
    if (r >= voff) return screen[r - voff];
    return hist[(htotal - (unsigned int)(voff - r)) % SB_LINES];
}
...
static void do_render(void) {
    ...
    int pos = 0;
    for (int r = 0; r < TH; r++) {
        const struct tcell *row = view_row(r);
        int c = 0;
        while (c < TW) {
            unsigned int cp = row[c].cp;
            unsigned char fg = row[c].fg;
            unsigned char bg = row[c].bg;
            if (cp == 0) { c++; continue; }
            int start = c;
            pos = 0;
            while (c < TW && row[c].cp && row[c].fg == fg && row[c].bg == bg) {
                pos += gui_utf8_encode(utfbuf + pos, row[c].cp);
                c++;
            }
            utfbuf[pos] = 0;
            gui_text(start * FONT_W, r * FONT_H, utfbuf,
                     xterm_rgb[fg], xterm_rgb[bg]);
        }
    }
    if (cursor_visible && sh_alive && voff == 0)
        gui_fill(ccol * FONT_W, crow * FONT_H + 14, FONT_W, 2, col_fg);
    gui_update();
}
...
static void term_out_byte(unsigned char b) {
    ...
    if (esc_state == 2) {
        ...
        if (pn < 0) { cursor_visible = (b == 'h'); return; }
        if (b == 'H') {
            crow = pr > 0 ? pr - 1 : 0;
            ccol = pn > 0 ? pn - 1 : 0;
            if (crow < 0) crow = 0;
        ...
        if (b == 'D') { ccol -= pn > 0 ? pn : 1; if (ccol < 0) ccol = 0; return; }
        if (b == 'C') { ccol += pn > 0 ? pn : 1; if (ccol >= TW) ccol = TW - 1; return; }
        return;
    }
...
static void send_key(unsigned int key) {
    char buf[4];
    switch (key) {
    case 0x0101: write(fd_in, "\x1b[A", 3); break;
    case 0x0102: write(fd_in, "\x1b[B", 3); break;
    case 0x0103: write(fd_in, "\x1b[D", 3); break;
    case 0x0104: write(fd_in, "\x1b[C", 3); break;
    case 0x0105: write(fd_in, "\x1b[H", 3); break;
    case 0x0106: write(fd_in, "\x1b[F", 3); break;
    case 0x0107: write(fd_in, "\x1b[3~", 4); break;
    ...
    default:
        if (key < 0x100) {
            ...
            int n = gui_utf8_encode(buf, (unsigned int)key);
...
static void spawn_sh(void) {
    ...
    int in[2], out[2];
    if (pipe(in) != 0) { fd_in = -1; fd_out = -1; return; }
    if (pipe(out) != 0) {
...
static void on_key(int k) {
    if (k == 0x0108 || k == 0x0109) {
        voff += (k == 0x0108) ? (TH - 2) : -(TH - 2);
        sb_clamp();
        return;
    }
    if (sh_alive) {
        if (voff != 0) voff = 0;
        send_key((unsigned int)k);
    } else if (k == '\r') {
        spawn_sh();
        voff = 0;
    }
...
void main(void) {
    ...
    struct gui_app app = {
        .title = "Terminal",
        .width = TW * FONT_W, .height = TH * FONT_H,
        .init = on_init,
        .render = do_render,
        .on_key = on_key,
        .on_scroll = on_scroll,
